 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Snap_Sniffs_Commenting_ClassCommentSniff extends Snap_Sniffs_Commenting_FileCommentSniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // No more than one blank line between short and long description.
        $long = $comment->getLongComment();
        if (empty($long) === false) {
            $between        = $comment->getWhiteSpaceBetween();
            $newlineBetween = substr_count($between, $phpcsFile->eolChar);
            if ($newlineBetween > 2) {
                $error = 'There must be no more than one blank line between descriptions in %s comments';
        ...
        // No more than one blank line before tags.
        $tags = $this->commentParser->getTagOrders();
        if (count($tags) > 1) {
            $newlineSpan = $comment->getNewlineAfter();
            if ($newlineSpan > 2) {
                $error = 'There must be no more than one blank line before the tags in %s comments';
                if ($long !== '') {
